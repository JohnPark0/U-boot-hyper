...
static void do_cp15_32(struct cpu_user_regs* regs, const union hsr hsr)
{
	...
	int regidx = cp64.reg1;		//reg1 or reg2?
	uint64_t* r = (uint64_t*)&regs->x0 + regidx;
	...
	{
	case HSR_CPREG64(SCTLR):
		TVM_EMUL(regs, hsr, *r, SCTLR);
		break;
	case HSR_CPREG64(TTBR0):
		TVM_EMUL(regs, hsr, *r, TTBR0);
		break;
	case HSR_CPREG64(TTBR1):
		TVM_EMUL(regs, hsr, *r, TTBR1);
		break;
	case HSR_CPREG64(TTBCR):
		TVM_EMUL(regs, hsr, *r, TTBCR);
		break;
	case HSR_CPREG64(DACR):
		TVM_EMUL(regs, hsr, *r, DACR);
		break;
	case HSR_CPREG64(DFSR):
		TVM_EMUL(regs, hsr, *r, DFSR);
		break;
	case HSR_CPREG64(IFSR):
		TVM_EMUL(regs, hsr, *r, IFSR);
		break;
	case HSR_CPREG64(DFAR):
		TVM_EMUL(regs, hsr, *r, DFAR);
		break;
	case HSR_CPREG64(IFAR):
		TVM_EMUL(regs, hsr, *r, IFAR);
		break;
	case HSR_CPREG64(ADFSR):
		TVM_EMUL(regs, hsr, *r, ADFSR);
		break;
	case HSR_CPREG64(AIFSR):
		TVM_EMUL(regs, hsr, *r, AIFSR);
		break;
	case HSR_CPREG64(MAIR0):
		TVM_EMUL(regs, hsr, *r, MAIR0);
		break;
	case HSR_CPREG64(MAIR1):
		TVM_EMUL(regs, hsr, *r, MAIR1);
		break;
	case HSR_CPREG64(AMAIR0):
		TVM_EMUL(regs, hsr, *r, AMAIR0);
		break;
	case HSR_CPREG64(AMAIR1):
		TVM_EMUL(regs, hsr, *r, AMAIR1);
		break;
	case HSR_CPREG64(CONTEXTIDR):
		TVM_EMUL(regs, hsr, *r, CONTEXTIDR);
		break;
	}
...
static void do_hvc32(struct cpu_user_regs* regs, const union hsr hsr)
{
	...
	case 1:
		...
		hvc_set_exec(regs->x0, regs->x1);
		break;
	case 2:
		/* set kernel ro memory */
		//hvc_set_ro(regs->r0, regs->r1);
		hvc_set_ro(regs->x0, regs->x1);
		break;
	case 3:
...
void do_trap_hyp(struct cpu_user_regs* regs)
{
	...
	/*  
	 *	READ_SYSREG32(HSR) = register uint32_t _r;
	 *	READ_SYSREG64(HSR) = register uint64_t _r; 
	 *	hsr.bits = register_t bits; = uint64_t
	*/ 
	const union hsr hsr = { .bits = READ_SYSREG64(HSR) };

	switch (hsr.ec) {
	//case HSR_EC_CP15_32:
	case HSR_EC_CP15_64:					//0x04
		nr_hyp_ctraps++;
		do_cp15_32(regs, hsr);
		break;
	case HSR_EC_HVC32:
